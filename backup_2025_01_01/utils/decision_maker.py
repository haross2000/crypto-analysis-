...
class DecisionMaker:
    ...
    def _get_decision(self, score: float) -> tuple[str, float]:
        ...
        base_confidence = min(abs(score - 50) * 2, 100) / 100
        
        # تحديد القرار
        if score >= 65:
            decision = 'شراء'
            # زيادة الثقة كلما زادت النتيجة عن 65
            confidence_boost = min((score - 65) / 35, 1) * 0.2
            confidence = base_confidence + confidence_boost
            
        elif score <= 35:
            decision = 'بيع'
            # زيادة الثقة كلما قلت النتيجة عن 35
            confidence_boost = min((35 - score) / 35, 1) * 0.2
            confidence = base_confidence + confidence_boost
            
        else:
            decision = 'انتظار'
            # تقليل الثقة في منطقة الانتظار
        ...
        elif confidence <= 0.3 and decision != 'انتظار':
            decision = 'انتظار'
    ...
    def _analyze_technical_signals(self, signals: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if 'MACD' in signals:
                ...
                macd_signal = float(signals.get('MACD_Signal', 0))
                macd_hist = float(signals.get('MACD_Hist', 0))
                
                # التحقق من صحة قيم MACD
                if (abs(macd) > self.thresholds['MACD']['min_signal'] and 
                    abs(macd) < self.thresholds['MACD']['max_signal'] and
                    abs(macd_hist) > self.thresholds['MACD']['min_hist']):
                    
                    # حساب قوة الإشارة (0-1)
                    signal_strength = min(abs(macd_hist) / 2, 1.0)
                    
                    # حساب النتيجة كنسبة مئوية
                    if macd > macd_signal and macd_hist > 0:
                        score += signal_strength * 50 * self.technical_weights['MACD']
